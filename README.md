...
| 레이어 | 인터랙션 범위 | 방식 |
|---|---|---|
| **헥스빈 지도** (~350m 셀) | 요일 멀티선택 × 시간 슬라이더 — 전 조합 | 원본 전량(슬림 JSON)을 브라우저에서 실시간 재집계 |
| **도로 플로우 지도** | 주중/주말 × 시간대 4구간(새벽/오전/오후/저녁) — 8슬라이스 | osmnx 최단경로 14,366건을 빌드 타임에 사전 집계 |

- 헥스빈은 트립 밀도 / 평균 속력 지표 전환 가능, 표본 5건 미만 셀은 반투명 처리
- 도로 플로우는 통행량을 선 굵기·밝기로 표현
- Plotly 차트 2종: 요일×시간 평균 속력 히트맵(전역), 시간대별 트립 수(선택 요일 연동)

## 주요 관찰

- **오전 9시가 가장 느리다** — 시간대 평균 속력 최저 11.8 km/h. 심야(0–5시) 19.2 km/h vs 주간(8–18시) 12.4 km/h.
- **트립은 퇴근길에 몰린다** — 18시 912건 피크. 최저인 새벽 5시(150건)의 약 6배.
- **주말이 주중보다 빠르다** — 주말 15.5 km/h vs 주중 13.9 km/h (+1.6 km/h). 주말의 심야 트립 비중(14.2%)이 평균을 끌어올린 몫도 있음.
- **장거리 트립은 고속도로 효과** — 직선거리 상위 10%(평균 12.9 km)의 평균 속력 24.7 km/h로 나머지(13.2 km/h)의 약 2배. 이 중 25.0%는 JFK, 32.8%는 라과디아 공항행.
...
1. **속력은 하한 추정치** — haversine 직선거리 / 소요시간 기준. 실주행 거리는 더 길므로 실제 속력은 이보다 높음.
2. **최단경로 ≠ 실주행 경로** — 도로 플로우는 OSM 도로망 최단경로로 추정한 것이며 기사가 실제 택한 경로가 아님.
3. **2016년 표본** — 현재 교통 상황과 다를 수 있음.
