...
- 헥스빈은 트립 밀도 / 평균 속력 지표 전환 가능, 표본 5건 미만 셀은 반투명 처리. 밀도 색상 스케일은 선택된 요일·시간 슬라이스의 분위수(p50/p85/p98)로 매번 동적 구성 — 희소 선택에서도 대비 유지
- 도로 플로우는 통행량을 선 굵기 + 청→적 발산 색으로 표현 (두꺼울수록 붉게, 얇을수록 파랗고 흐리게)
